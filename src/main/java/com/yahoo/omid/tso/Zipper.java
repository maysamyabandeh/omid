...
public class Zipper {
   ...
   */
   ...
   public void encodeFullAbort(ChannelBuffer buffer, long startTimestamp) {
      ...
      if (diff >= -16 && diff <= 15) {
         ...
         buffer.writeByte(TSOMessage.FullAbortReportByte);
         buffer.writeByte((byte) diff);
      } else {
   ...
   */
   ...
   private TSOMessage decodeFullAbort(byte diff) {
      ...
      return new FullAbortReport(lastFullAbortedTimestamp);
   }
   ...
   private FullAbortReport decodeFullAbort(byte type, ChannelBuffer buffer) {
       FullAbortReport msg;
